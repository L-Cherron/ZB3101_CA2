...
# Volcano plot with the gene name labels and -logp > 50
...
ggsave(here("results", "enhanced_volcano.png"), p1, width = 10, height = 12, units = "in", dpi = 300)
...
ggsave(here("results", "distrution_eed.png"), p1, width = 6, height = 5)
...
p1 <- ggplot(broadpeaks_WT_me3) +
  ggtitle("WT me3 Peaks") +
...
p2 <- ggplot(broadpeaks_KO_me3) +
    ggtitle("cKO me3 Peaks") +
    geom_segment(data = chrom_sizes,
               aes(x = chr, y = 0, yend = len),
               linewidth = 4.5, color = "white") +
  geom_segment(data=chrom_sizes,
...
ggsave(here("results", "me3peak_comparison.png"), plot_grid(p1, p2), width = 10, height = 6, units = "in", dpi = 300)
...
p1 <- ggplot(bind_rows(broadpeaks_KO_me3, broadpeaks_WT_me3) %>%
         filter(chrom==2) %>% 
  ...
  geom_segment(data=chrom_sizes %>% filter(chr==2),
               ...
               linewidth=4, color= "lightblue") +
  geom_point(aes(x=name,y=chromStart)) + 
  coord_flip() + 
  theme_tufte()

ggsave(here("results", "me3peak_comparison_chr2.png"), p1, width = 10, height = 3, units = "in", dpi = 300)
...
p1 <- ggplot() +
  ggtitle("WT (orange) vs cKO (light blue) H3K27ac peaks") +
  geom_segment(data=chrom_sizes,
               aes(x=chr, y=0, yend=len),
